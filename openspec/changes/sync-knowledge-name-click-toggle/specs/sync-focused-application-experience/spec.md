...
#### Scenario: Clicking node name toggles its checkbox
- **WHEN** a user clicks on a document or directory node's name (title text) in the source-selection tree
- **THEN** the node's checkbox state is toggled (checked becomes unchecked, unchecked becomes checked), the node becomes highlighted, and the uncheckedSyncedDocKeys state is updated consistently with the checkbox toggle
- **AND** the result is identical to clicking the node's checkbox directly
