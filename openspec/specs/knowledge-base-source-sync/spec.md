...
#### Scenario: Discovery retries transient document-info throttling
- **WHEN** sync-task creation hits a Feishu document info response with `code=99991400` while discovering selected knowledge-base documents
- **THEN** the backend retries that document-info lookup with bounded backoff before failing the discovery run
...
#### Scenario: Unchecked synced documents deleted before sync task creation
- **WHEN** the user clicks "Start Sync" and there are documents that are synced on disk but not in the current checked keys set
- **THEN** the system deletes those documents via `removeSyncedDocuments` before creating the new sync task
